...
if [ $# == 0 ] || [ $1 == "help" ]; then
    echo "Mode of use"
    echo "dataset.sh start [webapp-1 | webapp-2 | webapp-3]"
    echo "dataset.sh stop [webapp-1 | webapp-2 | webapp-3]"
    exit
else
    if [ $1 == 'start' ]; then
        case "$2" in 
            webapp-1) 
                echo "Running Webapp-1 in background spring and angular... Logs is saved in /home/dataset/logs" 
            ...
            webapp-2)
                echo "Running Webapp-2 in background spring and angular... Logs is saved in /home/dataset/logs" 
                cd /home/dataset/webapp-2/AMICOServer
                mvn spring-boot:run > /home/dataset/logs/backend-webapp-2 &
                echo $! > /var/log/backend-webapp-2-pid
                cd /home/dataset/webapp-2/Angular
                npm install --unsafe
                npx ng serve --host 0.0.0.0 > /home/dataset/logs/frontend-webapp-2 &
                echo $! > /var/log/backend-webapp-2-pid
                ;;

            webapp-3)
                echo "Running Webapp-3 in background spring... Logs is saved in /home/dataset/logs" 
